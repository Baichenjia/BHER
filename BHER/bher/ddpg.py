...
from BHER.bher.util import (
    import_function, store_args, flatten_grads, transitions_in_episode_batch)
from BHER.bher.normalizer import Normalizer
from BHER.bher.replay_buffer import ReplayBuffer
from BHER.common.mpi_adam import MpiAdam
from BHER.common.running_mean_std import RunningMeanStd
from BHER.common.schedules import LinearSchedule
...
class DDPG(object):
    ...
    def __init__(self, input_dims, buffer_size, hidden, layers, network_class, polyak, batch_size,
                 ...
                 sample_transitions, gamma, r_bias, bias_clip_low, bias_clip_high
                 , n_epochs, ismuti, reuse=False, **kwargs):
    ...
    def save_reward_pic(self, reward):
        ...
        plt.hist(reward, bins,alpha=0.5, weights= [1./ len(reward)] * len(reward))  # alpha设置透明度，0为完全透明
        font2 = {
                 'size': 18,
                 }
    ...
    def get_actions(self, o, ag, g, noise_eps=0., random_eps=0., use_target_net=False,
                    compute_Q=False, compute_r_bias = False):
    ...
    def recompute_reward(self,transitions):
        ...
        re_transitions['o'] = re_transitions['o'][:, :T, :].reshape(batch_size*T,self.dimo)
        re_transitions['ag'] = re_transitions['ag'][:, :T, :].reshape(batch_size*T,self.dimg)
        re_transitions['future_g'] = re_transitions['future_g'].reshape(batch_size*T,self.dimg)
        re_transitions['g'] = re_transitions['g'].reshape(batch_size*T,self.dimg)
        re_transitions['u'] = re_transitions['u'].reshape(batch_size*T,self.dimu)

        u1 = self.get_actions(re_transitions['o'], re_transitions['ag'], re_transitions['future_g'],
                              compute_r_bias=True)
        u2 = self.get_actions(re_transitions['o'], re_transitions['ag'], re_transitions['g'],
                              compute_r_bias=True)
        r_b = self.rb * (np.square(LA.norm(u2 - re_transitions['u'], axis=1)) - np.square(
            LA.norm(u1 - re_transitions['u'], axis=1)))
        ...
        if self.isMuti:
            transitions['r']  *= np.clip(e_r_b, self.bias_clip_low, self.bias_clip_high)
