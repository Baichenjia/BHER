...
import BHER.cher.experiment.config as config

from BHER.her.rollout import RolloutWorker
from BHER.her.util import mpi_fork
import BHER.cher.config_curriculum as config_cur
import datetime
...
@click.command()
@click.option('--env_name', type=str, default='FetchPickAndPlace-v1', help='the name of the OpenAI Gym environment that you want to train on')
@click.option('--logdir', type=str, default='../../result/cher/FetchPickAndPlace-1', help='the path to where logs and policy pickles should go. If not specified, creates a folder in /tmp/')
...
@click.option('--replay_strategy', type=click.Choice(['cher', 'none']), default='cher', help='the HER replay strategy to be used. "future" uses HER, "none" disables HER.')
